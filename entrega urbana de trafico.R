...
#espacio de trabajo
ruta <- "D:\\NIXON\\universidad\\programas universidad\\R\\practica\\correlacion\\03. URBANA_TRAFICO.xlsx"
df <- read_excel(ruta)
...
intento2011 <- df2011 %>% group_by(mes) %>% summarise( CEN_TRAF = mean(`CEN-TRAF`),  SUR_TRAF = mean(`SUR-TRAF`) , MED_PJIC = mean(`MED-PJIC`), Mes = first(MesL))
intento2012 <- df2012 %>% group_by(mes) %>% summarise( CEN_TRAF = mean(`CEN-TRAF`),  SUR_TRAF = mean(`SUR-TRAF`) , MED_PJIC = mean(`MED-PJIC`), Mes = first(MesL))
intento2013 <- df2013 %>% group_by(mes) %>% summarise( CEN_TRAF = mean(`CEN-TRAF`),  SUR_TRAF = mean(`SUR-TRAF`) , MED_PJIC = mean(`MED-PJIC`), Mes = first(MesL))
intento2014 <- df2014 %>% group_by(mes) %>% summarise( CEN_TRAF = mean(`CEN-TRAF`),  SUR_TRAF = mean(`SUR-TRAF`) , MED_PJIC = mean(`MED-PJIC`), Mes = first(MesL))
intento2015 <- df2015 %>% group_by(mes) %>% summarise( CEN_TRAF = mean(`CEN-TRAF`),  SUR_TRAF = mean(`SUR-TRAF`) , MED_PJIC = mean(`MED-PJIC`), Mes = first(MesL))
intento2016 <- df2016 %>% group_by(mes) %>% summarise( CEN_TRAF = mean(`CEN-TRAF`),  SUR_TRAF = mean(`SUR-TRAF`) , MED_PJIC = mean(`MED-PJIC`), Mes = first(MesL))
intento2017 <- df2017 %>% group_by(mes) %>% summarise( CEN_TRAF = mean(`CEN-TRAF`),  SUR_TRAF = mean(`SUR-TRAF`) , MED_PJIC = mean(`MED-PJIC`), Mes = first(MesL))
#intento2017 <- intento2017[intento2017$promedioBEME != "0",]
intento2018 <- df2018 %>% group_by(mes) %>% summarise( CEN_TRAF = mean(`CEN-TRAF`),  SUR_TRAF = mean(`SUR-TRAF`) , MED_PJIC = mean(`MED-PJIC`), Mes = first(MesL))
intento2019 <- df2019 %>% group_by(mes ) %>% summarise( CEN_TRAF = mean(`CEN-TRAF`),  SUR_TRAF = mean(`SUR-TRAF`) , MED_PJIC = mean(`MED-PJIC`), Mes = first(MesL))
intento2020 <- df2020 %>% group_by(mes) %>% summarise( CEN_TRAF = mean(`CEN-TRAF`),  SUR_TRAF = mean(`SUR-TRAF`) , MED_PJIC = mean(`MED-PJIC`), Mes = first(MesL))
intento2021 <- df2021 %>% group_by(mes) %>% summarise( CEN_TRAF = mean(`CEN-TRAF`),  SUR_TRAF = mean(`SUR-TRAF`) , MED_PJIC = mean(`MED-PJIC`), Mes = first(MesL))
...
g_2021 <- ggplot(intento2021, aes(x = mes))+
  ...
  geom_point(aes(y = SUR_TRAF,group = 1, color = "SUR_TRAF" ))+
  geom_point(aes(y = CEN_TRAF,group = 1, color = "CEN_TRAF" ))+
  geom_line(aes(y = SUR_TRAF,group = 1 , color = "SUR_TRAF"))+ 
  labs(title = "2021")+
  ylab("PM2.5	(µ/m3)")+
...
g_2015 <- ggplot(intento2015, aes(x = mes))+
  geom_line(aes(y  = CEN_TRAF, group = 1 , color = "CEN_TRAF"  ))+
  geom_point(aes(y = SUR_TRAF,group = 1, color = "SUR_TRAF" ))+
  geom_line(aes(y = MED_PJIC,group = 1 , color = "MED_PJIC"))+
  geom_point(aes(y = CEN_TRAF,group = 1, color = "CEN_TRAF" ))+
  geom_line(aes(y = SUR_TRAF,group = 1 , color = "SUR_TRAF"))+ 
  geom_point(aes(y = MED_PJIC,group = 1, color = "MED_PJIC" ))+
  labs(title = "2015")+
  ylab("PM2.5")+
  theme(axis.text = element_text(angle = 90))
...
AON = df %>% filter(año >= 2014 & año <= 2021 )

intentoAOV <- AON %>% group_by(mes) %>% group_by(mes) %>% summarise( CEN_TRAF = mean(`CEN-TRAF`),  SUR_TRAF = mean(`SUR-TRAF`))
