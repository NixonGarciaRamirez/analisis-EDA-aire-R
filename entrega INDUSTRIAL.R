...
#espacio de trabajo
ruta <- "D:\\NIXON\\universidad\\programas universidad\\R\\practica\\correlacion\\04. INDUSTRIAL.xlsx"
df <- read_excel(ruta)
...
intento2011 <- df2011 %>% group_by(mes) %>% summarise( ITA_CJUS = mean(`ITA-CJUS`),  MED_FISC = mean(`MED-FISC`) ,  Mes = first(MesL))
intento2012 <- df2012 %>% group_by(mes) %>% summarise( ITA_CJUS = mean(`ITA-CJUS`),  MED_FISC = mean(`MED-FISC`) ,  Mes = first(MesL))
intento2013 <- df2013 %>% group_by(mes) %>% summarise( ITA_CJUS = mean(`ITA-CJUS`),  MED_FISC = mean(`MED-FISC`) ,  Mes = first(MesL))
intento2014 <- df2014 %>% group_by(mes) %>% summarise( ITA_CJUS = mean(`ITA-CJUS`),  MED_FISC = mean(`MED-FISC`) ,  Mes = first(MesL))
intento2015 <- df2015 %>% group_by(mes) %>% summarise( ITA_CJUS = mean(`ITA-CJUS`),  MED_FISC = mean(`MED-FISC`) ,  Mes = first(MesL))
intento2016 <- df2016 %>% group_by(mes) %>% summarise( ITA_CJUS = mean(`ITA-CJUS`),  MED_FISC = mean(`MED-FISC`) ,  Mes = first(MesL))
intento2017 <- df2017 %>% group_by(mes) %>% summarise( ITA_CJUS = mean(`ITA-CJUS`),  MED_FISC = mean(`MED-FISC`) ,  Mes = first(MesL))
#intento2017 <- intento2017[intento2017$promedioBEME != "0",]
intento2018 <- df2018 %>% group_by(mes) %>% summarise( ITA_CJUS = mean(`ITA-CJUS`),  MED_FISC = mean(`MED-FISC`) ,  Mes = first(MesL))
intento2019 <- df2019 %>% group_by(mes ) %>% summarise( ITA_CJUS = mean(`ITA-CJUS`),  MED_FISC = mean(`MED-FISC`) ,  Mes = first(MesL))
intento2020 <- df2020 %>% group_by(mes) %>% summarise( ITA_CJUS = mean(`ITA-CJUS`),  MED_FISC = mean(`MED-FISC`) ,  Mes = first(MesL))
intento2021 <- df2021 %>% group_by(mes) %>% summarise( ITA_CJUS = mean(`ITA-CJUS`),  MED_FISC = mean(`MED-FISC`) ,  Mes = first(MesL))
...
g_2021 <- ggplot(intento2021, aes(x = mes))+
  ...
  geom_point(aes(y = MED_FISC,group = 1, color = "MED_FISC" ))+
  geom_point(aes(y = ITA_CJUS,group = 1, color = "ITA_CJUS" ))+
  geom_line(aes(y = MED_FISC,group = 1 , color = "MED_FISC"))+ 
  labs(title = "2021")+
  ylab("PM2.5	(µ/m3)")+
...
AON = df %>% filter(año >= 2020 & año <= 2022 )

intentoAOV <- AON %>% group_by(mes) %>% group_by(mes) %>% summarise( ITA_CJUS = mean(`ITA-CJUS`),  MED_FISC = mean(`MED-FISC`))
